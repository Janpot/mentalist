...
describe('.number', function () {
  ...
  it('("ZERO") should return 0', function () {
    process.env.ZERO = '0';
    var result = mentalist.number('ZERO');
    assert.strictEqual(result, 0);
  });
  
  it('("ZERO", 1) should return 0', function () {
    process.env.ZERO = '0';
    var result = mentalist.number('ZERO', 1);
    assert.strictEqual(result, 0);
  });
  
  it('("ONE", undefined) should return 1', function () {
    process.env.ONE = '1';
    var result = mentalist.number('ONE');
    assert.strictEqual(result, 1);
  });
  
  it('("ONE", 2) should return 1', function () {
    process.env.ONE = '1';
    var result = mentalist.number('ONE', 2);
    assert.strictEqual(result, 1);
  });
  
  it('("UNDEFINED", null) should return null', function () {
    delete process.env.UNDEFINED;
    var result = mentalist.number('UNDEFINED', null);
    assert.strictEqual(result, null);
  });
  
  it('("UNDEFINED", 0) should return 0', function () {
    delete process.env.UNDEFINED;
    var result = mentalist.number('UNDEFINED', 0);
    assert.strictEqual(result, 0);
  });
  
  it('("UNDEFINED", 1) should return 1', function () {
    delete process.env.UNDEFINED;
    var result = mentalist.number('UNDEFINED', 1);
    assert.strictEqual(result, 1);
  });
